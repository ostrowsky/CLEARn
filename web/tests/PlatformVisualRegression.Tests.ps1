...
Write-TestStep 'Visual regression compares local learner screens with checked-in mockup baselines'

$fixtureRoot = Join-Path $PSScriptRoot 'fixtures\visual-regression'
$baselineDir = Join-Path $fixtureRoot 'renders'
$screenDir = Join-Path $fixtureRoot 'screens'
Assert-True -Condition (Test-Path -LiteralPath (Join-Path $baselineDir '01-desktop-home.png')) -Message 'Checked-in visual baseline PNG files must be available for screenshot comparison.'
Assert-True -Condition (Test-Path -LiteralPath (Join-Path $screenDir '01-desktop-home.html')) -Message 'Checked-in visual screen HTML files must be available for exact-size baseline regeneration.'
