...
Assert-True -Condition ($clientSections -notmatch 'window\.location\.pathname === ''/sections''') -Message 'Learner home should not depend on the legacy /sections redirect.'
Assert-True -Condition ($clientSections -notmatch '<Redirect href="/"') -Message 'Learner home should render directly at / without a /sections redirect shim.'
...
Assert-Match -Actual $sharePreview -Pattern 'learnerUrl = "\$clientBaseUrl/"'
Assert-Match -Actual $sharePreview -Pattern 'Local learner preview: http://127\.0\.0\.1:\$clientPort/'
