...
- Vercel Analytics must be wired through a post-export `dist/index.html` injection, not through a Next.js-only layout, an Expo `+html.tsx` assumption, or a Metro-bundled analytics component, because the frontend is an Expo Router static export.
